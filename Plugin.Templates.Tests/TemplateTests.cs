...
namespace Linkoid.Repo.Plugin.Templates.Tests;

public class TemplateTests : TemplateTestBase
{
    [Test]
    [TestCase("HarmonyPlugin", "repoharmony", new[] { "-n", "Example.Plugin", "-A", "User" })]
    public async Task ProjectTemplate(string folderName, string shortName, string[] args)
    {
        ...
        {
            TemplatePath = FileLocator.GetTemplateLocation("Plugin.Templates", folderName),
